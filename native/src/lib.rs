...
fn app_pub_enc_key(mut cx: FunctionContext) -> JsResult<JsUndefined> {
    ...
    let cxp = Box::into_raw(Box::new(cx)) as *mut std::os::raw::c_void;
    // let cxp = Box::new(cx);
    // let cxp = Box::leak(cxp) as *mut std::os::raw::c_void;

    dbg!(cxp);

    unsafe {
        safe_app::ffi::crypto::app_pub_enc_key(app, cxp, o_cb);
    }
    extern "C" fn o_cb(
...
fn test_create_app(mut cx: FunctionContext) -> JsResult<JsUndefined> {
    ...
    let cxp = Box::new(cx);
    let cxp = Box::into_raw(cxp) as *mut std::os::raw::c_void;

    unsafe {
        safe_app::ffi::test_utils::test_create_app(app_id.as_ptr(), cxp, o_cb);
    }
    extern "C" fn o_cb(user_data: *mut std::os::raw::c_void, error: *const FfiResult, o_app: *mut App) {
        let cx: Box<FunctionContext> = unsafe { Box::from_raw(user_data as *mut _) };
        let mut cx = *cx;
